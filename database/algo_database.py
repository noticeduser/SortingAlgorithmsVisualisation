...
class AlgorithmDatabase:
    ...
    def append_database(self, algorithm, time_complexity, grid_size, sorting_time):
        self.cursor.execute(
            f"INSERT INTO algorithms VALUES ('{algorithm}', '{time_complexity}', {grid_size}, {sorting_time})"
        )
    ...
    def get_time_data(self):
        self.cursor.execute(
            "SELECT algorithm, grid_size, sorting_time FROM algorithms WHERE sorting_time = (SELECT MAX(sorting_time) FROM algorithms)"
        )
        slowest_algorithm = self.cursor.fetchall()

        self.cursor.execute(
            "SELECT algorithm, grid_size, sorting_time FROM algorithms WHERE sorting_time = (SELECT MIN(sorting_time) FROM algorithms)"
        )
